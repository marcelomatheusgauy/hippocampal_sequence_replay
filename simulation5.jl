...
############################################################
	...
	if 0==1
	#################################### old
	# Global inhibitory cells weights
	number_excitatory_targets = floor(Int, Ne*p)
	number_inhibitory_targets = floor(Int, Ni*p)
	global_inhibitory_excitatory_targets = zeros(Int, global_inhibitory_cells, number_excitatory_targets)
	global_inhibitory_inhibitory_targets = zeros(Int, global_inhibitory_cells, number_inhibitory_targets)
	# Global inhibitory cells spikes
	for dd = 1:global_inhibitory_cells
		global_inhibitory_excitatory_targets[dd,:] = randperm(Ne)[1:number_excitatory_targets]
		global_inhibitory_inhibitory_targets[dd,:] = Ne + randperm(Ni)[1:number_inhibitory_targets]
		...
		for cc = 1:Ncells
			if cc < Ne
				if global_inhibitory_excitatory_inhibitory_targets_to_cell[cc,dd] == 1
					...
					#println("auxiliary_excitatory_number_targets_per_global_inhibitory_cell = ", auxiliary_excitatory_number_targets_per_global_inhibitory_cell[dd])
					#println("global_inhibitory_excitatory_target_per_global_inhibitory_cell = ", global_inhibitory_excitatory_target_per_global_inhibitory_cell[auxiliary_excitatory_number_targets_per_global_inhibitory_cell[dd]])
		...
		for cc = 1:Ncells
			...
			for i in 1:size(ext_in_rates)[1]
				...
				if ext_in_rates[i,1] == t
					...
					if jextin <= 0.0001
						...
						if cc == 1
							#for dd = 1:global_inhibitory_cells
							#	nextin[dd] = ext_in_rates[i,2] + rand(expdist)/default_rextin
								#println(nextin[dd])
								#ns_global_inhibitory_cells[dd] += 1
								#times_global_inhibitory_cells[dd, ns_global_inhibitory_cells[dd]] = nextin[dd]
								#in_spike_count_per_step_per_pattern[number_patterns + 2, tt] += 1
							#end
						end
					end
					#println(jextin)
			...
			if(inhibitory_stimulus_off)
				if (auxiliary_pattern_recovery_count[auxiliary_pattern_index] < 2)
					if(tt > auxiliary_time[auxiliary_pattern_index] + 120) ### assumes inhibitory stimulus is turned off only after 12ms -- ## change the 120 to make I_G off longer
						if (spike_count_over_time_per_pattern[auxiliary_pattern_index+1,tt] > 10) #this number might have to be adapted with the system
							...
							if(auxiliary_pattern_recovery_count[auxiliary_pattern_index] == 2)
								...
								#for dd = 1:global_inhibitory_cells
								#	nextin[dd] = t + rand(expdist)/rextin
									#println(nextin[dd])
								#	ns_global_inhibitory_cells[dd] += 1
								#	times_global_inhibitory_cells[dd, ns_global_inhibitory_cells[dd]] = nextin[dd]
								#	in_spike_count_per_step_per_pattern[number_patterns + 2, tt] += 1
