...
# 6. this simulation should have the start (first pattern) be synchronous and activity then proceeds synchronously
...
function sim(weightpars,membranepars,synapsepars,stimpars,simpars,inhibition_1_pars, number_patterns, weights_to_place, weights) #runs simulation given weight matrix and populations
	...
	####################################getting weights matrix from encoding part
	###########################################################################
	...
	excitatory_indices = 1:(number_patterns*Ne)
	weights[excitatory_indices, :] = weights[excitatory_indices, :]*ach_factor
