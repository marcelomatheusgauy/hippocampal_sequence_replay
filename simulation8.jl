...
# 3. The stimulus signal (changed external rate) only goes to pattern A. (MG: What did this mean????)
# 4. We send a common external inhibitory signal (parameter inhibition_1_pars) to both a_ex and b_ex such that activation of E[i] should not trigger E[i+1] despite
#    E[i] sending input to E[i+1]. Also, the external inhibition should not kill pattern E[i] as well when it is active.
# 5. If we turn off the external inhibition then E[i+1] can turn active (ext_inhibition_1_min_weight.jl should confirm this)
# 6. In this simulation we have added a two pattern detector (new inhibitory population) which is only activated if both E[i] and E[i+1] are co-active.
# 7. this simulation should have the start be synchronous and activity then proceeds synchronously

include("afferent_recurrent_neuron.jl")

# The file which should be used to call this one is reversalreplay.jl
# this is adapted from simulation6.jl
